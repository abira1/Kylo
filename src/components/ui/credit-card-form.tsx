  ...
  return (
    <section className={`ccp ${className}`}>
      <div className="wrap">
        ...
        <form className="form" onSubmit={handleSubmit} noValidate>
          <div>
            ...
            <input id="number" inputMode="numeric" autoComplete="cc-number" placeholder="1234 5678 9012 3456"
              value={formatNumberSpaces(number)} onChange={(e) => setNumber(clampDigits(e.target.value, 19))}
              onFocus={() => setFocusField("number")} onBlur={() => setFocusField(null)} aria-invalid={!validity.number} />
            {!validity.number && number.length >= 13 && <small className="err">Card number looks invalid</small>}
          </div>
          <div>
            <label htmlFor="holder">Card Holder</label>
            <input id="holder" type="text" autoComplete="cc-name" placeholder="JANE DOE" value={holder}
              onChange={(e) => setHolder(e.target.value.toUpperCase())} onFocus={() => setFocusField("holder")}
              onBlur={() => setFocusField(null)} aria-invalid={!validity.holder} />
          </div>
          <div className="filed__group">
            <div>
              <label>Expiration Date</label>
              <div className="filed__date">
                <select value={month || ""} onChange={(e) => setMonth(e.target.value)} onFocus={() => setFocusField("expire")} onBlur={() => setFocusField(null)} aria-invalid={!validity.month}>
                  <option value="" disabled>Month</option>
                  {Array.from({ length: 12 }, (_, i) => String(i + 1).padStart(2, "0")).map((m) => <option key={m} value={m}>{m}</option>)}
                </select>
                <select value={year || ""} onChange={(e) => setYear(e.target.value)} onFocus={() => setFocusField("expire")} onBlur={() => setFocusField(null)} aria-invalid={!validity.year}>
            ...
            <div>
              ...
              <input id="cvv" inputMode="numeric" autoComplete="cc-csc" placeholder="***" value={cvv}
                onChange={(e) => setCVV(clampDigits(e.target.value, 4))} onFocus={() => setFocusField("cvv")} onBlur={() => setFocusField(null)} aria-invalid={!validity.cvv} />
            </div>
          </div>
          {showSubmit && (
      ...
      <style>{`
        ...
        .ccp .card { position: relative; width: 100%; max-width: 420px; margin: 0 auto; transform-style: preserve-3d; transition: .8s; perspective: 1000px; }
        .ccp .card.flip { transform: rotateY(180deg); }
        .ccp .card__front, .ccp .card__back { width: 100%; max-width: 420px; height: 233px; border-radius: 20px; padding: 24px 30px 30px; background: linear-gradient(to right bottom, #323941, #061018); box-shadow: 0 33px 50px -15px rgba(50,55,63,.66); color: #fff; overflow: hidden; margin: 0 auto; backface-visibility: hidden; position: relative; }
        @media (max-width: 450px) { .ccp .card__front, .ccp .card__back { padding: 12px 14px 16px; height: 206px; } .ccp #highlight.highlight__number { width: 300px; left: 14px; } .ccp #highlight.highlight__holder { width: 220px; left: 14px; } .ccp #highlight.highlight__expire { left: 280px; } .ccp #highlight.highlight__cvv { width: 330px; left: 14px; } }
        .ccp .card__back { position: absolute; top: 0; left: 0; transform: rotateY(180deg); padding: 24px 0 0; }
        .ccp .card__front::before, .ccp .card__back::before { content: ""; position: absolute; border: 16px solid var(--ring1); border-radius: 100%; left: -17%; top: -45px; height: 300px; width: 300px; filter: blur(13px); }
        .ccp .card__front::after, .ccp .card__back::after { content: ""; position: absolute; border: 16px solid var(--ring2); border-radius: 100%; width: 300px; top: 55%; left: -200px; height: 300px; filter: blur(13px); }
        ...
        .ccp .card__header { display: flex; align-items: center; justify-content: space-between; font-weight: 600; margin-bottom: 32px; position: relative; z-index: 1; }
        ...
        .ccp .card__number .digit { display: flex; flex-direction: column; height: 33px; line-height: 33px; transition: transform .2s; }
        ...
        @media (max-width: 560px) { .ccp .filed__group { grid-template-columns: 1fr; } }
        .ccp .filed__date { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
        .ccp .err { color: #b42318; font-size: 12px; margin-top: 4px; }
        .ccp .submit { margin-top: 8px; height: 48px; border: none; border-radius: 10px; background: #0d0c22; color: #fff; font-weight: 600; cursor: pointer; }
        .ccp .submit:disabled { opacity: .6; cursor: not-allowed; }
      `}</style>
    </section>
  );
};
